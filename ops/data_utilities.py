...
from ops.augmentations import blur, box, misalign, missing, pixel # warp
...
def affinitize(img, dst, dtype=np.uint8):
    """
...
def derive_affinities(affinity, label_volume, long_range=False):
    """Derive affinities from label_volume."""
    assert label_volume.dtype == np.uint8
    distances = np.rot90(np.eye(affinity, dtype=np.uint8))
    if long_range:
        distances.append((0,0,3))
        distances.append((0,3,0))
        distances.append((2,0,0))
        distances.append((0,0,9))
        distances.append((0,9,0))
        distances.append((3,0,0))
        distances.append((0,0,27))
        distances.append((0,27,0))
        distances.append((4,0,0))
    ground_truth_affinities = np.ndarray((len(distances),)+label_volume.shape, dtype=np.uint8)
    for i in range(len(distances)):
        ground_truth_affinities[i,:] = affinitize(label_volume, dst=distances[i])[:]
    return ground_truth_affinities.transpose(1, 2, 3, 0)
...
def apply_augmentations(volume, label, augmentations, input_shape, label_shape):
