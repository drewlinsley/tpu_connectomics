...
import numpy as np
from ops.augmentations.aug_utils import check_tensor
        ...
        if self.do_augment:
            for k, v in sample.iteritems():
                ...
                if 1:
                    ...
                    new_data[:, ymin:ymax, xmin:xmax, :] = data[
                        :, ymin:ymax, xmin:xmax, :]
                    # Slip.
                    xmin = max(-self.x_t, 0)
                    ymin = max(-self.y_t, 0)
                    ...
                    ymax = min(-self.y_t, 0) + y
                    pvot = self.pivot[k]
                    new_data[pvot, ymin:ymax, xmin:xmax, :] = data[
                        pvot, ymin:ymax, xmin:xmax, :]
                else:
                    # Copy upper box.
                    xmin = max(self.x_t, 0)
                    ymin = max(self.y_t, 0)
                    ...
                    ymax = min(self.y_t, 0) + y
                    pvot = self.pivot[k]
                    new_data[0:pvot, ymin:ymax, xmin:xmax, :] = data[
                        0:pvot, ymin:ymax, xmin:xmax, :]
                    # Copy lower box.
                    xmin = max(-self.x_t, 0)
                    ymin = max(-self.y_t, 0)
                    xmax = min(-self.x_t, 0) + x
